"""
core/graph.py  (UPDATED)
──────────────────────────
Builds the LangGraph StateGraph for AXIOM — now with three new nodes:

  [security_agent]   → OSV + PyPI vulnerability scanning (after rules, before resolver)
  [connector_agent]  → Input/output transformer — generates adapter shims (after axiom)
  Token tracking     → wired into resolver + axiom via LLM call hooks
...
FULL PIPELINE:
  ...
  [Rules Engine]       →  [rules_agent]       is_installed, api_overlap, license
                          [security_agent]    NEW: OSV CVE scan + PyPI freshness ← ★
  ...
  [Stage 3 verdict]    →  [axiom_agent]       weighted scoring + AST rewrite
                          [connector_agent]   NEW: API adapter shim generation  ← ★
...
Graph topology:
  ...
  [rules]          ← deterministic, no LLM
     │
  [security]       ← NEW: OSV + PyPI scan (parallel HTTP, no LLM)
     │
  ...
  [axiom]          ← LLM #2: weighted scoring + AST rewrite
     │
  [connector]      ← NEW: LLM #3: generate API adapter shims
     │
    [END]
"""
